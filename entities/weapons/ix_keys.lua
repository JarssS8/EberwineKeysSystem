...
SWEP.Instructions = "Click Izquierdo: Bloquear\nClick Derecho: Desbloquear"
SWEP.Purpose = ""
SWEP.Drop = false
...
function SWEP:PrimaryAttack()
	...
	--[[
		Locks the entity if the contiditon fits:
			1. The entity is door and client has access to the door or The character has the key of the door..
			2. The entity is vehicle and the "owner" variable is same as client's character ID.
	--]]
	if IsValid(entity) and self:GetOwner():GetCharacter() and ((entity:IsDoor() and entity:CheckDoorAccess(self:GetOwner()) or self:GetOwner():GetCharacter():HasDoorAccess(entity)) or (entity:IsVehicle() and entity.CPPIGetOwner and entity:CPPIGetOwner() == self:GetOwner())) then
...
function SWEP:SecondaryAttack()
	...
	--
	if IsValid(entity) and ((entity:IsDoor() and entity:CheckDoorAccess(self:GetOwner()) or self:GetOwner():GetCharacter():HasDoorAccess(entity)) or (entity:IsVehicle() and entity.CPPIGetOwner and entity:CPPIGetOwner() == self:GetOwner())) then
		self:GetOwner():SetAction(
			"@unlocking",
			time,
			function()
